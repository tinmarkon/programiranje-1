...
let priblizek_korena x = int_of_float (floor (sqrt (float_of_int x)))
...
let izpisi_soda_liha (list : int list) : unit =
  list |> List.filter (fun x -> if x mod 2 = 0 then true else false) |> List.iter print_int;
  list |> List.filter (fun x -> if x mod 2 = 1 then true else false) |> List.iter print_int
...
[*----------------------------------------------------------------------------*)
let rec alternirajoci_konstruktorji (list : 'a option list) : bool = 
  match list with
  | [] | _ :: [] -> true
  | Some _ :: None :: xs -> alternirajoci_konstruktorji (None :: xs)
  | None :: Some x :: xs -> alternirajoci_konstruktorji (Some x :: xs)
  | _ -> false (*Ne razumem tega robnega primera*)
...
let najmanjsi_rezultat x fs = 
  let rec aux_najmanjsi acc i fs  = 
    match acc, fs with
    | acc, [] -> Option.map snd acc
    | None , f :: fs -> aux_najmanjsi (Some (f x, i)) (i + 1) fs
    | Some (y, j), f :: fs -> aux_najmanjsi (Some (min (y, j) (f x, i))) (i + 1) fs
in aux_najmanjsi None 0 fs
