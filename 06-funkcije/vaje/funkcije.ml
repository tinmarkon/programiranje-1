...
let mapi f list =  
  let rec aux_map list count =
    match list with
    | [] -> []
    | x :: xs -> f x count :: (aux_map xs (count + 1))
  in aux_map list 0
...
let unzip_tlrec lst =
  let rec aux_unzip lst acc1 acc2 =
  match lst with
  | [] -> acc1, acc2
  | (x, y) :: xs -> aux_unzip xs (x :: acc1) (y :: acc2)
in aux_unzip (reverse' lst) [] []
...
let apply_sequence f x n = 
  let rec aux_apply f x n acc = 
    if n < 0 then acc else aux_apply f (f x) (n - 1) (x :: acc)
  in reverse' (aux_apply f x n [])
(*----------------------------------------------------------------------------*]
...
| [] -> []
| x :: tail -> if f x then x :: filter f tail else filter f tail
...
let rec exists f list = 
  match list with
  | [] -> false
  | x :: tail -> if f x then true else exists f tail
...
let rec first f default list = 
  match list with
  | [] -> default
  | x :: tail -> if f x then x else first f default tail 