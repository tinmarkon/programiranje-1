...
let rec repeat = 
  function
  | (element, stevilo) -> if stevilo <= 0 then [] else element :: repeat (element, stevilo - 1)
...
[*----------------------------------------------------------------------------*)
let mapi f list =
  let rec mapi list n acc =
    match list with
    | [] -> reverse' acc
    | x :: xs -> mapi xs (n + 1) (f x n :: acc)
  in mapi list 0 []
...
let rec unzip sez =
  ...
  | [] -> ([], [])
  | (x, y) :: ostanek ->
    let (list1, list2) = unzip ostanek in
    (x :: list1, y :: list2) 
...
let rec loop pogoj funkcija x =
  if pogoj x then loop pogoj funkcija (funkcija x) else x
...
let rec fold_left_no_acc funkcija = function
...
let filter f list = 
  let rec filter_aux list acc =
    match list with
    | [] -> reverse' acc
    | x :: xs -> if f x then filter_aux xs (x :: acc) else filter_aux xs acc
  in filter_aux list []
...
let rec exists funkcija seznam =
  match seznam with
  | [] -> false
  | x :: xs -> if funkcija x then true else exists funkcija xs
